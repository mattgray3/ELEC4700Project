...
RL = 0.9;%0.9i; % Reflectoin coefficients for L and R sides
RR = 0.9;%0.9i;
...
Nt =  100*floor(2*Nz); % total # of time steps
...
beta_r = 0; %DETUNING Real Beta value Milestone 2 Real component adds a "Twist"
...
figure('Name','State Space Analysis of Laser Output');
plot(OutputRE, OutputIM, 'r');
hold on;
scatter(OutputRE(1), OutputIM(1), 100,'b','filled');
scatter(OutputRE(end), OutputIM(end), 100, 'g', 'filled');
hold off;

grid on;
xlabel('OutputR REAL Part');
ylabel('OutputR IMAGINARY Part');
title('State Space Analysis of Laser OutputR (RE Vs Im)');
