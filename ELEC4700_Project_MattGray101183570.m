...
kappaStart = 1/3; % Where Grating Starts
kappaStop = 2/3; %Where Grating ends
...
InputParasL.wg = 0; %5e-13 ; %pusle width
InputParasL.phi = 0;
...
Nt =  floor(2*Nz); %floor(2*Nz);ceil(3000e-12 / dt); % total # of time steps
...
gain = vg * 10e-20;% Milestone 6
eVol = 1.5e-10 * c_q;% Milestone 6
Ion = L*0.25e-9;% Milestone 6
Ioff = 3e-9;% Milestone 6
I_off = 1;% Milestone 6
I_on = 4;% Milestone 6
taun = 5e-9;% Milestone 6
Zg = sqrt(c_mu_0/c_eps_0)/n_g;% Milestone 6
EtoP = 1/(Zg*f0*vg*1e-2*c_hb);% Milestone 6
alpha = 0;
N = ones(size(z)) * Ntr;
Nave = zeros(1, Nt);
...
for i = 2:Nt
    ...
    Stim = gain .* (N-Ntr) .* S;
    Stim = max(Stim, -N/2); %Milestone 6

    N = (N + dt*(I_injv/eVol - Stim))./(1 + dt/taun);%Milestone 6
   
    Nave(i) = mean(N);

    fprintf('t = %.2e s, I_injv = %.2e, Stim = %.2e, N = %.2e\n', t, I_injv, mean(Stim), mean(N));
    ...
    if mod(i, plotN) == 0
        ...
        subplot(3,2,2)
        plot(z * 1e4, N, 'b', 'LineWidth', 1.5); 
        xlabel('z (\mum)')
        ylabel('N')
        title('Carrier Density Across z')
        ylim([min(N) * 0.9, max(N) * 1.1])

        subplot(3,2,3)
        plot(time * 1e12, Nave, 'm');hold on
        xlabel('time (ps)')
        ylabel('Nave')
